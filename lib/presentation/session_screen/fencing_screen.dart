...
class FencingDashboard extends StatefulWidget {
  ...
  final FencingDataModel? dataModel;
...
class _FencingDashboardState extends State<FencingDashboard>
  ...
  static const double pulleyCircumference = 94.25; // محيط البكرة بالسم
  ...
  void initState() {
    ...
    _updateData(widget.dataModel?? FencingDataModel(direction: 0, speed: 0));
  }
  ...
  void didUpdateWidget(FencingDashboard oldWidget) {
    ...
    if (oldWidget.dataModel != widget.dataModel) {
      _updateData(widget.dataModel?? FencingDataModel(direction: 0, speed: 0)); // تحديث البيانات لما الـ dataModel يتغير
    }
  }

  void _updateData(FencingDataModel data) {
    int direction = data.direction;
    // افترض إن speed هنا عدد الدورات في الثانية، نحولها لمسافة/ثانية (سم/ث)
    double speed = (data.speed) * direction;
